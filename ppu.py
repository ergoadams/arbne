...
class ppu:
    ...
    def ppuRead(self, addr, readOnly = False):
        ...
        if self.cartridge.ppuRead(self.addr, readOnly):
            self.data = self.cartridge.CHRmemory[self.cartridge.mapped_addr]
        elif self.addr >= 0 and self.addr <= 0x1FFF:
    ...
    def ppuWrite(self, addr, data):
        self.addr = addr & 0x3FFF
        if not self.cartridge.ppuWrite(self.addr, data):
            if self.addr >= 0 and self.addr <= 0x1FFF:
    ...
    def clock(self):
        if (self.scanline >= -1) and (self.scanline < 240):
            if not self.scanline and not self.cycle and self.odd_frame and (self.maskreg_render_background or self.maskreg_render_sprites):
                self.cycle = 1
            ...
            if (self.cycle >= 2 and self.cycle < 258) or (self.cycle >= 321 and self.cycle < 338):
                if self.maskreg_render_background:
                    ...
                    self.bg_shifter_attrib_lo <<= 1
                    self.bg_shifter_attrib_hi <<= 1
                if self.maskreg_render_sprites and self.cycle and self.cycle < 258:
                    for i in range(self.sprite_count):
                        if self.spriteScanline[(i * 4) + 3]:
                            self.spriteScanline[(i * 4) + 3] -= 1
                        else:
            ...
            if self.cycle == 256 and (self.maskreg_render_background or self.maskreg_render_sprites):
            ...
            if (self.cycle == 338) or (self.cycle == 340):
                ...
                self.vram_addr |= (self.vram_addr_unused << 15)
                self.bg_next_tile_id = self.ppuRead(0x2000 | (self.vram_addr & 0x0FFF))
            if self.scanline == -1 and self.cycle >= 280 and self.cycle < 305 and (self.maskreg_render_background or self.maskreg_render_sprites):
        ...
        if self.maskreg_render_background and (self.maskreg_render_background_left or self.cycle >= 9):
        ...
        if self.maskreg_render_sprites and (self.maskreg_render_sprites_left or self.cycle >= 9):
            self.spriteZeroBeingRendered = False
            for i in range(self.sprite_count):
                self.tempaddr = i * 4
                if not self.spriteScanline[self.tempaddr + 3]:
                    ...
                    self.fg_priority = 1 if not (self.spriteScanline[self.tempaddr + 2]  & 0x20) else 0
                    if self.fg_pixel:
                        if not i:
                            self.spriteZeroBeingRendered = True
                        break
        self.pixel, self.palette = 0, 0
        if not self.bg_pixel and not self.fg_pixel:
            self.pixel, self.palette = 0, 0
        elif not self.bg_pixel and self.fg_pixel:
            self.pixel, self.palette = self.fg_pixel, self.fg_palette
        elif self.bg_pixel and not self.fg_pixel:
            self.pixel, self.palette = self.bg_pixel, self.bg_palette
        elif self.bg_pixel and self.fg_pixel:
            self.pixel, self.palette = (self.fg_pixel, self.fg_palette) if self.fg_priority else (self.bg_pixel, self.bg_palette)
            if self.spriteZeroHitPossible and self.spriteZeroBeingRendered and self.maskreg_render_background & self.maskreg_render_sprites:
                if not (self.maskreg_render_background_left | self.maskreg_render_sprites_left) and self.cycle >= 9 and self.cycle < 256:
        ...
        if self.cycle >= 341:
            ...
            if self.scanline >= 261:
                self.scanline = -1
                self.odd_frame = False if self.odd_frame else True
                self.framecomplete = True
                self.framecount += 1
                #Render frame
                self.img = pygame.image.frombuffer(bytearray(self.screenarray), (256, 240), "RGB")
                self.window.blit(pygame.transform.scale(self.img, (256*self.scaling_factor, 240*self.scaling_factor)), (0, 0), pygame.Rect(0, 0, 256*self.scaling_factor, 240*self.scaling_factor))
