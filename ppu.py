...
class ppu:
	def __init__(self):
		self.running = True
		self.tblName = np.zeros([2, 1024], dtype = int)
		self.tblPattern = np.zeros([2, 4096], dtype = int)
		self.tblPalette = np.zeros(32, dtype = int)
		...
		self.controller = [0, 0]
		self.screen = np.zeros([256, 240], dtype = int)
		...
		self.nmi = False
		self.oam = np.zeros(256, dtype = int)
		self.oam_addr = 0
		self.spriteScanline = np.zeros(32, dtype = int)
		self.sprite_count = 0
		self.sprite_shifter_pattern_lo = np.zeros(8, dtype = int)
		self.sprite_shifter_pattern_hi = np.zeros(8, dtype = int)
	...
	def clock(self):
		if (self.scanline >= -1) and (self.scanline < 240):
			...
			if self.scanline == -1 and self.cycle == 1:
				...
				self.sprite_shifter_pattern_lo = self.sprite_shifter_pattern_hi = np.zeros(8, dtype = int)
			...
			if self.cycle == 257 and self.scanline >= 0:
				self.spriteScanline = np.full(32, 255)
				self.sprite_count = 0
				self.sprite_shifter_pattern_lo = np.zeros(8, dtype = int)
				self.sprite_shifter_pattern_hi = np.zeros(8, dtype = int)
